...
export async function listUsers(options?: {
  ...
  if (orderBy === "name") {
    ...
    orderByClause =
      order === "asc" ? asc(users.createdAt) : desc(users.createdAt);
  } else if (orderBy === "updatedAt") {
    orderByClause =
      order === "asc" ? asc(users.updatedAt) : desc(users.updatedAt);
  } else {
    orderByClause =
      order === "asc" ? asc(users.createdAt) : desc(users.createdAt);
  }
...
export async function countUsers(options?: {
  ...
  if (conditions.length > 0) {
    ...
    result = await db.select({ count: sql<number>`count(*)` }).from(users);
  }
...
export async function listNewsletterSubscribers(options?: {
  ...
  if (orderBy === "email") {
    orderByClause =
      order === "asc" ? asc(newsletter.email) : desc(newsletter.email);
  } else {
    orderByClause =
      order === "asc" ? asc(newsletter.createdAt) : desc(newsletter.createdAt);
  }
...
export async function listProducts(options?: {
  ...
  if (minPrice !== undefined) {
    ...
    const minPriceStr =
      typeof minPrice === "number" ? minPrice.toString() : minPrice;
  ...
  if (maxPrice !== undefined) {
    // Convert to string for numeric comparison
    const maxPriceStr =
      typeof maxPrice === "number" ? maxPrice.toString() : maxPrice;
...
export async function countProducts(options?: {
  ...
  if (conditions.length > 0) {
    ...
    result = await db.select({ count: sql<number>`count(*)` }).from(products);
  }
...
export async function listOrders(options?: {
  ...
  if (orderBy === "createdAt") {
    ...
    orderByClause =
      order === "asc" ? asc(orders.updatedAt) : desc(orders.updatedAt);
  } else if (orderBy === "totalAmount") {
    orderByClause =
      order === "asc" ? asc(orders.totalAmount) : desc(orders.totalAmount);
...
export async function countOrders(options?: {
  ...
  if (conditions.length > 0) {
    ...
    result = await db.select({ count: sql<number>`count(*)` }).from(orders);
  }
